...
from backend.app.models.risk_model import BASELINE, FEATURES, WEIGHTS
from backend.app.models.schemas import ShapContribution
...
def explain(final_score: int, feats: dict[str, float], top_k: int = 7) -> list[ShapContribution]:
    deltas = {f: WEIGHTS[f] * (feats.get(f, 0.0) - BASELINE[f]) for f in FEATURES}
