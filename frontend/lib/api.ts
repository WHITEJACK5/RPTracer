...
export async function evaluateRisk(
  ...
  if (!res.ok) {
    throw new Error(String((data as unknown as { detail?: string })?.detail ?? `${res.status}`));
  }
...
export async function fetchPresets(): Promise<Record<string, Preset> | null> {
  try {
    return await http<Record<string, Preset>>("/api/v1/presets");
  } catch {
...
export async function fetchModelReport(): Promise<ModelReport> {
  return http<ModelReport>("/api/v1/model/report");
}

export async function fetchLedgerStats(): Promise<LedgerStat> {
  return http<LedgerStat>("/api/v1/ledger/stats");
}

export async function fetchLedger(limit = 100): Promise<LedgerEntry[]> {
  return http<LedgerEntry[]>(`/api/v1/ledger?limit=${limit}`);
}

export async function fetchAnalysts(): Promise<Analyst[]> {
  try {
    return await http<Analyst[]>("/api/v1/analysts");
  } catch {
    return BUNDLED_ANALYSTS;
  }
}

export async function fetchHealth(): Promise<{ status: string } | null> {
  try {
    return await http<{ status: string }>("/healthz");
  } catch {
    return null;
  }
}
...
export const BUNDLED_ANALYSTS: Analyst[] = [
  { id: "a1", name: "A. Rao", role: "Lead Risk Analyst", status: "online" },
  { id: "a2", name: "K. Mehta", role: "Graph Investigator", status: "investigating" },
  { id: "a3", name: "S. Nair", role: "ML Ops", status: "online" },
  { id: "a4", name: "D. Iyer", role: "Compliance", status: "offline" },
  { id: "a5", name: "R. Shah", role: "Fraud SME", status: "online" },
];
